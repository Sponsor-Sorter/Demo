...
// Followers total (optional – only runs if followersTotal.js was included)
function updateFollowersTotalSafe(platform, count) {
  try {
    const fn = window.__followersTotal?.update;
    if (typeof fn === 'function') fn(platform, count);
  } catch { /* noop */ }
}
...
async function updateSummaryStats() {
  ...
  if (actEl) actEl.textContent = active.length ?? 0;

  const completed = offers.filter(o => ['completed', 'review_completed'].includes(o.status));
  if (compEl) compEl.textContent = completed.length ?? 0;
...
async function loadYouTubeStats() {
  ...
  if (data.success && data.stats) {
    ...
    updateFollowersTotalSafe('youtube', Number(data.stats.subscriberCount) || 0);
...
async function loadTwitchStats() {
  ...
  try {
    ...
    if (followers != null) {
      ...
      updateFollowersTotalSafe('twitch', Number(followers) || 0);
    }
...
async function loadInstagramStats() {
  ...
  try {
    ...
    updateFollowersTotalSafe('instagram', Number(profile.followers_count) || 0);
...
async function loadFacebookStats() {
  ...
  try {
    ...
    if (followers != null) {
      fbEls.followers.textContent = fmtNum(followers);
      updateFollowersTotalSafe('facebook', Number(followers) || 0);
    }
